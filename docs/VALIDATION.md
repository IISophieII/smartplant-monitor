...
- 本地 8 项测试通过，包含事件合并、级别升级、恢复、重复确认、重开数据库、多设备隔离和 API 校验；Ruff 通过。
- 真实浏览器验证时间轴、悬停与键盘读数、告警生成/确认/恢复、中英文文本及手机布局。
- 不改变健康评分公式；告警没有去抖或自动保留期限，详细语义见 README。

## 双语版本验证（2026-09-13）

- 本地 6 项 pytest 测试通过，Ruff 与 JavaScript 语法检查通过。
- 实际浏览器验证英文默认界面、中文切换、刷新后记住语言、双语故障解释及恢复操作。
- 两种语言的 390px 手机布局无页面横向溢出，无 JavaScript 运行错误；中英文实际截图已更新。
